...
class AccountIvaFile(models.Model):
    ...
    def _prepare_invoice_line(self, data = [None,0,0,0], sign = 1):
    ...
    def _prepare_invoice_lines(self, monto_neto = 0, monto_no_gravado = 0, monto_exento = 0, monto_iva = 0, monto_total = 0):
        ...
        if monto_neto != 0 or monto_exento != 0 or monto_no_gravado != 0:
            data_items = [[self.product_vat_id,monto_neto,monto_total],[self.product_novat_id,monto_no_gravado,0],[self.product_exempt_id,monto_exento,0]]
        else:
            data_items = [[self.product_vat_id,monto_total,0]]
        for product,amount,total_amt in data_items:
            if amount > 0:
    ...
    def btn_process_file(self):
        ...
        for i,items in enumerate(csv_reader):
            ...
            if items[6] == 'CUIT':
                ...
                identification_type_id = self.env.ref('l10n_ar.it_dni').id
            if not partner_id:
                vals_partner = {
                        'name': razonsocial,
            ...
            if items[11] != '':
                invoice_line_ids = self._prepare_invoice_lines(float(items[11]),float(items[12]),float(items[13]),float(items[14]),float(items[15]))
            else:
                invoice_line_ids = self._prepare_invoice_lines(0,0,0,0,float(items[15]))
            vals_move = {
                    'move_type': move_type,
