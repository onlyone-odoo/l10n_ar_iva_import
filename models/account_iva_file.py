...
class AccountIvaFile(models.Model):
    ...
    product_novat_id = fields.Many2one(
        "product.product",
        string="Producto No Gravado",
        states={"draft": [("readonly", False)]},
    )
    product_exempt_id = fields.Many2one(
        "product.product",
        string="Producto Exento",
        states={"draft": [("readonly", False)]},
    )
    product_other_taxes_id = fields.Many2one(
        "product.product",
        string="Producto Otros Tributos",
        states={"draft": [("readonly", False)]},
    )
    date = fields.Date("Fecha Ingreso", default=fields.Date.today(), readonly=True)
    iva_file = fields.Binary(
        "Archivo", tracking=True, states={"draft": [("readonly", False)]}
    )
    ...
    partner_ids = fields.One2many(
        ...
        inverse_name="account_iva_file_id",
        string="Proveedores creados",
        readonly=True,
    )
    ...
    def _prepare_invoice_lines(
        ...
        if monto_neto != 0 or monto_exento != 0 or monto_no_gravado != 0:
            data_items = [
                [self.product_vat_id, monto_neto, monto_total, monto_iva],
                [self.product_novat_id, monto_no_gravado, 0, 0],
                [self.product_exempt_id, monto_exento, 0, 0],
            ]
        else:
                ...
                invoice_lines.append(
                    (
                        ...
                        self._prepare_invoice_line(
                            [product, amount, total_amt, monto_iva], 1
                        ),
                    )
                )
    ...
    def btn_process_file(self):
        ...
        for i, items in enumerate(csv_reader):
            ...
            doc_type_id = self.env["l10n_latam.document.type"].search(
                [("code", "=", doc_type[0])]
            )
